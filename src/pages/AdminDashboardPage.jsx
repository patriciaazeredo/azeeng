...
const AdminDashboardPage = () => {
  ...
  const fetchContactMessages = useCallback(async () => {
    ...
    try {
      ...
      toast({
        ...
        description:
          error.message || "Não foi possível carregar as mensagens de contato.",
        variant: "destructive",
      });
    } finally {
  ...
  const fetchPortfolioItems = useCallback(async () => {
    ...
    try {
      ...
      toast({
        ...
        description:
          error.message || "Não foi possível carregar os itens do portfólio.",
        variant: "destructive",
      });
    } finally {
      setLoadingPortfolio(false);
  ...
  useEffect(() => {
    ...
    if (user) {
      // Only fetch portfolio if user is available
      fetchPortfolioItems();
    }
            ...
            <Card className="shadow-xl">
              <CardHeader>
                <CardTitle className="text-2xl text-sky-600 flex items-center">
                  <MessageSquare className="mr-2 h-6 w-6" /> Mensagens de
                  Contato Recebidas
                </CardTitle>
                <CardDescription>
                  Visualize as mensagens enviadas através do formulário de
                  contato do site.
                </CardDescription>
              </CardHeader>
              <CardContent>
                          ...
                          <TableRow key={msg.id}>
                            ...
                            <TableCell className="max-w-xs">
                              <div className="flex flex-col">
                                <span
                                  className={`${
                                    expandedMessages[msg.id] ? "" : "truncate"
                                  }`}
                                >
                                  {msg.message}
                                </span>
                                {msg.message.length > 100 && (
                                  <button
                                    ...
                                    className="text-sky-600 text-sm mt-1 hover:underline self-start"
                                  >
          ...
          <TabsContent value="portfolio">
            <div className="grid lg:grid-cols-3 gap-8 items-start">
              <div className="lg:col-span-1">
                <PortfolioForm
                  onPortfolioItemAdded={handlePortfolioItemAdded}
                />
              </div>
              <div className="lg:col-span-2">
                <Card className="shadow-xl">
                  <CardHeader>
                    ...
                    <CardDescription>
                      Gerencie os projetos exibidos na seção de portfólio do
                      site.
                    </CardDescription>
                  </CardHeader>
                  <CardContent>
                    {loadingPortfolio && portfolioItems.length === 0 && user ? (
                      ...
                      <PortfolioList
                        ...
                        onItemsUpdate={handlePortfolioItemsUpdate}
                      />
                    )}
                  </CardContent>
                </Card>
