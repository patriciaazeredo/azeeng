        ...
        >
          <div className="container mx-auto px-4">
            <div className="flex items-center justify-between h-20">
              <Link to="/" onClick={() => document.getElementById('hero')?.scrollIntoView({ behavior: 'smooth' })} className="flex items-center space-x-2">
                <Building size={32} className="text-sky-600" />
                <span className="text-2xl font-bold text-sky-700">AZEENG</span>
              </Link>
              ...
              <div className="md:hidden flex items-center">
                <Button onClick={() => setIsOpen(!isOpen)} variant="ghost" size="icon" className="text-slate-700 hover:text-sky-600">
          ...
          >
            <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3">
              {navItems.map(item => (
                <MobileNavLink key={item.label} to={item.to} onClick={() => setIsOpen(false)}>{item.label}</MobileNavLink>
              ))}
              {user ? (
                <>
                  <MobileNavLink to="/admin/dashboard" isExternal={true} onClick={() => setIsOpen(false)}>
                    <ShieldCheck className="mr-2 h-5 w-5 inline-block" /> Painel Admin
                  </MobileNavLink>
                  <button
                 ...
                 <MobileNavLink to="/admin/login" isExternal={true} onClick={() => setIsOpen(false)}>
                    Acesso Admin
                  </MobileNavLink>
              )}
            </div>
